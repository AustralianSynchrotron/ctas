...
int main(int argc, char *argv[]) { {
  ...
  if (args.imgs.size() == 1) { // sinogram
    ...
    auto saveMe = [&args,&cent,&rec,&recr](int num){
      if ( !rec || !args.outmask.size() )
        return;
    ...
    auto saveMe = [&args,&pr0,&pr180,&cent](int num){
      if (!args.outmask.size())
        return;
      ...
      result=0;
      result(all, blitz::Range(0,ish(1)-1)) += shft > 0 ? pr0 : pr180.reverse(blitz::secondDim);
      result.reverse(blitz::secondDim)(all, blitz::Range(0,ish(1)-1)) += shft > 0 ? pr180 : pr0.reverse(blitz::secondDim);
      result /= 2;
